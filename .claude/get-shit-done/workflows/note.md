...
- **Global scope**: `/home/bill/source/DoML/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/bill/source/DoML/.claude/notes/`)
...
2. Glob `/home/bill/source/DoML/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/bill/source/DoML/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/bill/source/DoML/.claude/notes/` — works in any directory
